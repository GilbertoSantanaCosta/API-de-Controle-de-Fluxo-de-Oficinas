...
public class CadastrarClienteOficina implements InterfaceCommandOficina {

	private ClienteOficinaDAO clienteoficinaDAO;
	
	
	public CadastrarClienteOficina(ClienteOficinaDAO clienteoficinaDAO) {
		super();
	...
	public String Execute(HttpServletRequest request,
		...
		request.setAttribute("titulo", "Cadastro Sistema Automotivo");
		...
		try {
			...
			request.setAttribute("mensagem", "Problemas com data!");
			e1.printStackTrace();
		}
		...
		try {
			clienteoficinaDAO.Salvar(cliente);
			request.setAttribute("mensagem", "Cliente ("+cliente.getNome()+")gravado com sucesso!");
		} catch (Exception e) {
			request.setAttribute("mensagem","Problemas com a gravação2: "+e.getMessage());
			e.printStackTrace();
		}
